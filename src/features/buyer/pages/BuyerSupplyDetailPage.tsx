...
export function BuyerSupplyDetailPage() {
  ...
  const { register, handleSubmit, reset, setError, control, formState: { errors } } = useForm<ReservationFormValues>({
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      const response = await createReservation.mutateAsync({ volume_kg: values.volume_kg, notes: values.notes.trim() || null })
