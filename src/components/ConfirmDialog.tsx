    ...
    >
      <div className="p-5 sm:p-6">
        <div className="flex items-start gap-4">
          ...
          <button className="grid size-9 shrink-0 place-items-center rounded-lg text-ink-soft hover:bg-panel-alt" type="button" aria-label="Tutup dialog" disabled={pending} onClick={onCancel}><X size={18} /></button>
        </div>

        {error && <p className="mt-4 rounded-lg bg-danger-soft p-3 text-sm text-danger" role="alert">{error}</p>}

        <div className="mt-6 grid grid-cols-2 gap-3">
          <button ref={cancelRef} className="min-h-11 rounded-lg border border-line bg-white px-4 text-sm font-semibold text-ink hover:bg-panel-alt" type="button" disabled={pending} onClick={onCancel}>{cancelLabel}</button>
